...
export default class MonitorScreen extends EventEmitter {
    ...
    createExperienceLauncher() {
        const React    = require('react');
        const ReactDOM = require('react-dom');
        const { ExperienceLauncher } = require('../UI/components/ExperienceLauncher');

        const container = document.createElement('div');
        container.style.width    = this.screenSize.width  + 'px';
        container.style.height   = this.screenSize.height + 'px';
        container.style.overflow = 'hidden';
        container.id = 'computer-screen';

        ReactDOM.render(React.createElement(ExperienceLauncher), container);

        // Capture-phase listeners set inComputer=true before the bubble-phase
        // document listener in initializeScreenEvents() reads the flag.
